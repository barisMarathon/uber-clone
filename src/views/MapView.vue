  ...
  <div id="MapView">
    ...
    <div id="VehicleSelection" class="w-full">
      ...
      <div class="scrollSection">
        <div :class="[selectedRide === 'UberX' ? 'bg-custom-gray' : '']" @click="selectRide('UberX')">
        ...
        <div class="">
          <div class="flex items-center px-4 py-5" :class="[selectedRide === 'UberComfort' ? 'bg-custom-gray' : '']"
            @click="selectRide('UberComfort')">
            <img width="75" src="img/uber/comfort.png" alt="">
            <div class="w-full ml-3">
              <div class="flex items-center justify-between">
                <div class="text-2xl mb-1">UberComfort</div>
                <div class="text-xl">${{ calculatePrice(1.1, distance.value) }}</div>
              </div>
              <div class="text-gray-500 ">{{ duration.text }}</div>
            </div>
          </div>
        ...
        <div class="">
          <div class="flex items-center px-4 py-5" :class="[selectedRide === 'UberXL' ? 'bg-custom-gray' : '']"
            @click="selectRide('UberXL')">
            <img width="75" src="img/uber/uberxl.png">
            <div class="w-full ml-3">
              <div class="flex items-center justify-between">
                <div class="text-2xl mb-1">UberXL</div>
                <div class="text-xl">${{ calculatePrice(1.2, distance.value) }}</div>
              </div>
              <div class="text-gray-500 ">{{ duration.text }}</div>
            </div>
          </div>
      ...
      <div class="flex items-center bg-white justify-center py-6 px-4 w-full absolute bottom-0 shadow-inner">
        <button @click="acceptRide" class="bg-black text-2xl text-white py-4 px-4 rounded-sm w-full">
          Confirm Uber
        </button>
      </div>
    </div>
